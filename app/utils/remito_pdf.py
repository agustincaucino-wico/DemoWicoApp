...
def build_fuel_load_remito_pdf(fuel_load):
    ...
    pdf.setLineWidth(1)
    pdf.rect(content_left, content_bottom, content_width, content_height, stroke=1, fill=0)
    ...
    pdf.drawString(left_x + 7 * mm, header_top - 41 * mm, "I.V.A. RESPONSABLE INSCRIPTO")
    ...
    pdf.drawRightString(right_x + right_w - 6 * mm, header_top - 12 * mm, f"N {fuel_load.id}")
    ...
    pdf.line(label_x + 18 * mm, row1_y - 1 * mm, content_right - 4 * mm, row1_y - 1 * mm)
    pdf.drawString(
        label_x + 20 * mm,
        row1_y,
    ...
    pdf.line(org_label_x + 24 * mm, row2_y - 1 * mm, content_right - 4 * mm, row2_y - 1 * mm)
    ...
    pdf.line(label_x + 36 * mm, plate_y - 1 * mm, content_right - 4 * mm, plate_y - 1 * mm)
    pdf.drawString(
        label_x + 38 * mm,
        plate_y,
    ...
    header_row_h = 7 * mm
    pdf.setFillColor(colors.HexColor("#1B2F5B"))
    pdf.rect(content_left, table_top - header_row_h, content_width, header_row_h, stroke=0, fill=1)
    ...
    pdf.line(content_left + left_col_w, table_bottom, content_left + left_col_w, table_top)
    ...
    pdf.setFont("Helvetica", 9)
    pdf.drawString(content_left + 3 * mm, row0_bottom + 2 * mm, _format_currency(operation_amount))
    pdf.drawString(content_left + left_col_w + 3 * mm, row0_bottom + 2 * mm, description)
